...
import { Trophy, AlertTriangle, TrendingUp, Share2, Download, Copy, Dumbbell, Percent, Shuffle, Swords, Activity } from 'lucide-react';
...
const WeeklyRecap = ({ league, rosters, users, players, currentWeek }) => {
    ...
    const copyToClipboard = () => {
        ...
        else if (stats.matchupHighlight) {
            if (stats.matchupHighlight.type === 'STRUGGLE') {
                text += `🚽 The Struggle Bus:\n${stats.matchupHighlight.winner} (${stats.matchupHighlight.score}) scraped by against ${stats.matchupHighlight.loser}.\n\n`;
            } else if (stats.matchupHighlight.type === 'CLOSE') {
                text += `😅 The Heart Attack:\n${stats.matchupHighlight.winner} edged out ${stats.matchupHighlight.loser} by just ${stats.matchupHighlight.margin} pts.\n\n`;
            } else {
                text += `😤 The Beatdown:\n${stats.matchupHighlight.winner} destroyed ${stats.matchupHighlight.loser} by ${stats.matchupHighlight.margin} pts.\n\n`;
            }
        }
        ...
        <div className="space-y-6">
            ...
            <div ref={captureRef} className="p-8 rounded-xl bg-slate-950 border border-slate-800 shadow-2xl relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
                    {/* 1. The Robbery OR Tank Bowl Fallback */}
                    {stats.robbery ? (
                        <Card className="border-red-900/50 bg-red-950/20 backdrop-blur-sm">
                            ...
                            <CardContent>
                                <p className="text-slate-300 text-sm md:text-base">
                                    Scored <span className="font-bold text-white">{stats.robbery.score}</span> points (would beat <span className="font-bold text-red-400">{stats.robbery.percentile}%</span> of league) yet lost to {stats.robbery.opponent}. Call the cops.
                                </p>
                            </CardContent>
                        </Card>
                    ) : stats.matchupHighlight ? (
                        <Card className={`backdrop-blur-sm ${stats.matchupHighlight.type === 'STRUGGLE' ? 'border-slate-700 bg-slate-800/40' :
                            stats.matchupHighlight.type === 'CLOSE' ? 'border-rose-900/50 bg-rose-950/20' :
                                'border-blue-900/50 bg-blue-950/20' // BLOWOUT
                            }`}>
                            <CardHeader className="pb-2">
                                <div className={`flex items-center gap-2 mb-2 ${stats.matchupHighlight.type === 'STRUGGLE' ? 'text-slate-400' :
                                    stats.matchupHighlight.type === 'CLOSE' ? 'text-rose-500' :
                                        'text-blue-500'
                                    }`}>
                                    {stats.matchupHighlight.type === 'STRUGGLE' ? <Dumbbell className="w-5 h-5" /> :
                                        stats.matchupHighlight.type === 'CLOSE' ? <Activity className="w-5 h-5" /> :
                                            <Swords className="w-5 h-5" />
                                    }
                                    <span className="font-bold uppercase tracking-wider text-xs">{stats.matchupHighlight.title}</span>
                                </div>
                                <CardTitle className="text-xl md:text-2xl">
                                    {stats.matchupHighlight.type === 'STRUGGLE' ? 'A Win is a Win?' :
                                        stats.matchupHighlight.type === 'CLOSE' ? 'Too Close for Comfort' :
                                            'Absolute Destruction'}
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                                <p className="text-slate-300 text-sm md:text-base">
                                    {stats.matchupHighlight.type === 'STRUGGLE' ? (
                                        <>
                                            {stats.matchupHighlight.winner} scraped by with just <span className="font-bold text-white">{stats.matchupHighlight.score}</span> points against {stats.matchupHighlight.loser}.
                                        </>
                                    ) : stats.matchupHighlight.type === 'CLOSE' ? (
                                        <>
                                            {stats.matchupHighlight.winner} edged out {stats.matchupHighlight.loser} by just <span className="font-bold text-white">{stats.matchupHighlight.margin}</span> points.
                                        </>
                                    ) : (
                                        <>
                                            Stop, he's already dead! {stats.matchupHighlight.winner} destroyed {stats.matchupHighlight.loser} by <span className="font-bold text-white">{stats.matchupHighlight.margin}</span> points.
                                        </>
                                    )}
                                </p>
                            </CardContent>
