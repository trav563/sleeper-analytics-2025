                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center gap-3">
                                                        <div className="relative shrink-0">
                                                            <img src={avatarUrl(row.currentOwner?.avatar)} className="w-8 h-8 rounded-full border border-slate-600 object-cover" />
                                                            {row.isTraded && (
                                                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 rounded-full border-2 border-slate-900"></div>
                                                            )}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <div className={`font-bold truncate ${row.isTraded ? 'text-blue-300' : 'text-slate-200'}`}>
                                                                {displayTeamName(row.currentOwner)}
                                                            </div>
                                                            {row.isTraded && (
                                                                <div className="text-[10px] text-blue-400/70 uppercase tracking-wider font-bold sm:hidden truncate">
